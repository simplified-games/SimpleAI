    ...
    return `
      <div class="model-card${isSelected ? " selected" : ""}" onclick="selectModel('${m.id}')">
        ...
        <div class="model-card-footer">
          ...
          <span class="model-card-cost">${m.costPerKTokens}t/1K</span>
        </div>
      </div>
    `;
  }).join("");
...
window.selectModel = (modelId) => {
  ...
  // Update reasoning toggle availability
...
async function sendMessage() {
  ...
  const isImageRequest = /\b(generate|create|draw|make|paint|render)\b.*\b(image|picture|photo|art|illustration)\b/i.test(userText) && !capturedImage;
